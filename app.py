...
# ── Encrypt ───────────────────────────────────────────────────────────────
    ...
    if mode == "Auto-generate key":
        ...
        st.caption("⚠️ Share via a separate channel — never alongside the encrypted message.")
    else:
...
if "result" in st.session_state:
    st.success(st.session_state.get("result_label", "Output") + ":")
    ...
    if operation == "Encrypt":
        ...
        st.info("Combined token (key + message merged — safe to share as one string):")
